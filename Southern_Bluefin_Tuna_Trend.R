...
#isolate data for Limosa haemastica from LPI dataset (ID: 19333)
Tuna <- LPI_Long[LPI_Long$ID %in% c(19333),]

#convert character data to integer
Tuna$Pop <- as.numeric(Tuna$Pop)
Tuna$Year <- as.numeric(Tuna$Year)

#plot with linear regression, standard error and GAM
#legend symbols must be within aes to show on plot
ggplot(data=subset(Tuna, !is.na(Tuna$Pop)), aes(x = Year, y = Pop)) + 
  ...
  ggtitle("Simple Population Time Series Analysis of the Southern Bluefin Tuna (Thunnus Maccoyii)") +
  ylab("Abundance (Metric Tonnes)") +
  guides(color = guide_legend(override.aes = list(shape = NA, size = 2)))+ scale_y_continuous(labels = comma)
