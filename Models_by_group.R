...
#read in xlsx file
lpd <- read_xlsx("/Users/le19811/Documents/MRes/LPI_Data/LPI.xlsx")
bs <- read_xlsx("/Users/le19811/Documents/MRes/Supplementary_Datasets/Body_Mass_means.xlsx")
...
ggpairs(vif.test)
cor.test(vif.test$lognat_bs, vif.test$Latitude, method="pearson")
...
###################################################################################
ta = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = terr.amph, 
             na.action = "na.fail", family = "truncated_compois")

fa = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = fresh.amph, 
             na.action = "na.fail", family = "truncated_compois")

tr = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = terr.rept, 
             na.action = "na.fail", family = "truncated_compois")

mr = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = marine.rept, 
             na.action = "na.fail", family = "truncated_compois")

fr = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = fresh.rept, 
             na.action = "na.fail", family = "truncated_compois")

tm = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = terr.mamm, 
             na.action = "na.fail", family = "truncated_compois")

mm = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = marine.mamm, 
             na.action = "na.fail", family = "truncated_compois")

fm = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
               (1|Binomial), data = fresh.mamm, 
             na.action = "na.fail", family = "truncated_compois")

tbi = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
                (1|Binomial), data = terr.bird, 
              na.action = "na.fail", family = "truncated_compois")

mbi = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
                (1|Binomial), data = marine.bird, 
              na.action = "na.fail", family = "truncated_compois")

fbi2 = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
                 (1|Binomial), data = fresh.bird, 
               na.action = "na.fail", family = "truncated_compois")

mcf = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
                (1|Binomial), data = marine.carti, 
              na.action = "na.fail", family = "truncated_compois")

mbf = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
                (1|Binomial), data = marine.bony, 
              na.action = "na.fail", family = "truncated_compois")

fbf = glmmTMB(nplus1 ~ (lognat_bs + ns(Latitude, df = 4))^2 + 
                (1|Binomial), data = fresh.bony, 
              na.action = "na.fail", family = "truncated_compois")
...
bs.lat.terr.rept <- ggpredict(terr.rept.best, 
                              terms = c("Latitude",
                                        "lognat_bs [quart2]"), 
                              interval = "confidence")
...
marine.rept.best <- glmmTMB(nplus1 ~ ns(Latitude, df = 4) + 
                              (1|Binomial), data = marine.rept, 
                            family = "truncated_compois")
...
marine.mamm.best <- glmmTMB(nplus1 ~ lognat_bs + 
                              (1|Binomial), data = marine.mamm, 
                            family = "truncated_compois")
...
lat.fresh.bony.best <- ggpredict(fresh.bony.best, 
                                 ...
                                 interval = "confidence")
###################################################################################
#create function appending ggpredict onjects with index column specifying original
#data frame
###################################################################################
append <- function(...){
  args <- list(...)
...
lat.fresh <- append(lat.fresh.amph.best, lat.fresh.bird.best, lat.fresh.bony.best,
                    lat.fresh.mamm.best, lat.fresh.rept.best)
###################################################################################
...
bs.lat.terr <- append(bs.lat.terr.bird, bs.lat.terr.rept)
...
bs.terr[bs.terr$source == "bs.terr.amph.best", ][, "sig"] <- "Significant"
...
bs.fresh[bs.fresh$source == "bs.fresh.amph.best", ][, "sig"] <- "Significant"
bs.fresh[bs.fresh$source == "bs.fresh.rept.best", ][, "sig"] <- "Non-significant"

bs.lat.marine$source <- as.character(bs.lat.marine$source)
bs.lat.marine$source[bs.lat.marine$source == 'bs.lat.marine.bony'] <- "Bony Fish"
bs.lat.marine$source[bs.lat.marine$source == 'bs.lat.marine.carti'] <- "Cartilaginous Fish"
bs.lat.marine$source[bs.lat.marine$source == 'bs.lat.marine.bird'] <- "Birds"
bs.lat.marine$source[bs.lat.marine$source == 'bs.lat.marine.rept'] <- "Reptiles"
bs.lat.marine$source[bs.lat.marine$source == 'bs.lat.marine.amph'] <- "Amphibians"
bs.lat.marine$source[bs.lat.marine$source == 'bs.lat.marine.mamm'] <- "Mammals"
...
bs.l.t <- ggplot(bs.lat.terr, aes(x, predicted-1)) +
  ...
  scale_colour_manual(labels = c("First", "Median", "Third"),
                      values = c("goldenrod1", "forestgreen", "hotpink4", "goldenrod1", "forestgreen", "hotpink4")) +
  scale_fill_manual(values = c("goldenrod1", "forestgreen", "hotpink4", "goldenrod1", "forestgreen", "hotpink4")) +
  ggtitle("Terrestrial") +
  theme_bw() +
  ylim(-1, 4) +
  scale_x_continuous(breaks=pretty_breaks(n=10)) +   
  ...
  guides(color=guide_legend(override.aes=list(fill=c("goldenrod1", 
                                                     "forestgreen", "hotpink4", "goldenrod1", "forestgreen", "hotpink4")))) 
...
grid.arrange(bs.l.t, bs.l.m, bs.l.f, nrow = 3)
